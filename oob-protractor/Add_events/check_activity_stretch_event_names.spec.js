...
describe ("Check if fields exist, and if they are named properly - ", function(){
    ...
    it ("On event-add/menu2/ (clik on Stretch), check if fields exist, and if they are named properly", function() {
        console.log("On event-add/menu2/ (clik on Stretch, check if fields exist, and if they are named properly");
        //klik na Activity
        eventAdd.categoris.get(0).click();
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);

        //klik na clik on Stretch
        eventAdd.categoris.get(7).click();
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);
        expect(eventAdd.categoris.get(0).isDisplayed()).toBeTruthy();
        //Nacin da proveri text
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);
        expect(eventAdd.categoris.get(0).getText()).toBe('Stretching');
        //klik na Stretching
        eventAdd.categoris.get(0).click();
