...
describe ("Check if fields exist, and if they are named properly - ", function(){
    ...
    it ("On event-add/menu1/ (clik on Reports), check if fields exist, and if they are named properly", function() {
        console.log("On event-add/menu1/ (clik on Reports), check if fields exist, and if they are named properly");
        ...
        expect(eventAdd.categoris.get(0).getText()).toBe('Motivation');
        expect(eventAdd.categoris.get(1).getText()).toBe('Stress');
        expect(eventAdd.categoris.get(2).getText()).toBe('Weight');
        expect(eventAdd.categoris.get(3).getText()).toBe('Blood pressure');
        expect(eventAdd.categoris.get(4).getText()).toBe('Sleep');
        expect(eventAdd.categoris.get(5).getText()).toBe('Resting heart rate');
        expect(eventAdd.categoris.get(6).getText()).toBe('Injury');
        expect(eventAdd.categoris.get(7).getText()).toBe('Sickness');
    });

    it ("On /event-add/form/ (clik on Motivation), check if Create event field exist, and go on previous page, and chech " +
        "fields exist on event-add/menu/, and if they are named properly", function() {
        console.log("On /event-add/form/  (clik on Motivation), check if Create event field exist, and go on previous page, and chech " +
