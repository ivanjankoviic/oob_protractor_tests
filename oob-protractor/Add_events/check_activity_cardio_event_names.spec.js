...
describe ("Check if fields exist, and if they are named properly - ", function(){
    ...
    it ("On event-add/menu2/ (clik on Cardio), check if fields exist, and if they are named properly", function() {
        console.log("On event-add/menu2/ (clik on Cardio, check if fields exist, and if they are named properly");

        //klik na Activity
        eventAdd.categoris.get(0).click();
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);

        //klik na clik on Cardio
        ...
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);
        expect(eventAdd.categoris.get(0).getText()).toBe('Crosstrainer');
        expect(eventAdd.categoris.get(1).getText()).toBe('Stairclimber');
        expect(eventAdd.categoris.get(2).getText()).toBe('Rope skipping');
        expect(eventAdd.categoris.get(3).getText()).toBe('Cardio');

    });

    it ("On /event-add/form/ (clik on Crosstrainer), check if Create event field exist, and go on previous page, and check " +
        "fields exist on event-add/menu/, and if they are named properly", function() {
        console.log("On /event-add/form/  (clik on Crosstrainer), check if Create event field exist, and go on previous page, and check " +
            "fields exist on event-add/menu/, and if they are named properly");

        //klik na Crosstrainer
        eventAdd.categoris.get(0).click();
