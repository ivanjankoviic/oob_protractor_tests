...
describe ("Check if fields exist, and if they are named properly - ", function(){
    ...
    it ("On event-add/menu2/ (clik on Dinner), check if fields exist, and if they are named properly", function() {
        console.log("On event-add/menu2/ (clik on Dinner), check if fields exist, and if they are named properly");
        ...
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);
        expect(eventAdd.categoris.get(0).getText()).toBe('Bread');
        expect(eventAdd.categoris.get(1).getText()).toBe('Fish');
        expect(eventAdd.categoris.get(2).getText()).toBe('Fruit');
        expect(eventAdd.categoris.get(3).getText()).toBe('Meat');
        expect(eventAdd.categoris.get(4).getText()).toBe('Pasta');
        expect(eventAdd.categoris.get(5).getText()).toBe('Rice');
        expect(eventAdd.categoris.get(6).getText()).toBe('Salad');
        expect(eventAdd.categoris.get(7).getText()).toBe('Vegetables');
        expect(eventAdd.categoris.get(8).getText()).toBe('Soup');
        expect(eventAdd.categoris.get(9).getText()).toBe('Others');
        //klik na Bread
