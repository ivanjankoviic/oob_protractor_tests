...
describe ("Create 3 different events in present past and future, and delete them - ", function(){
    ...
    it ("Checking stats before and after adding event and after report that event. Event is 'Weight''", function() {
        console.log("Checking stats before and after adding event and after report that event. Event is 'Weight'");
        ...
        //klik na znak pitanja
        stats.questionMark.click();
        // check alert
        browser.wait(EC.presenceOf(element(by.css(".popup-head"))), 5000);
        expect(element(by.css(".popup-head")).getText()).toEqual('HHS Score');
        browser.wait(EC.presenceOf(element(by.css(".popup-body"))), 5000);
        expect(element(by.css(".popup-body")).getText()).toEqual('Average Happy, Healthy, Strong score for chosen period');
        //click OK button
