...
describe ("Check if fields exist, and if they are named properly - ", function(){
    ...
    it ("On event-add/menu2/ (click on Run race, check if fields exist, and if they are named properly", function() {
        console.log("On event-add/menu2/ (click on Run race, check if fields exist, and if they are named properly");
        //klik na Goal
        eventAdd.categoris.get(1).click();
        browser.wait(EC.presenceOf(eventAdd.categoris), 5000);

        //klik na clik on Run race
        ...
        browser.wait(EC.presenceOf(eventAdd.categoris), 10000);
        expect(eventAdd.categoris.get(0).getText()).toBe('5 km');
        expect(eventAdd.categoris.get(1).getText()).toBe('10 km');
        expect(eventAdd.categoris.get(2).getText()).toBe('Halfmarathon');
        expect(eventAdd.categoris.get(3).getText()).toBe('Marathon');
        expect(eventAdd.categoris.get(4).getText()).toBe('Other distance');
        //klik na 5km
        eventAdd.categoris.get(0).click();
