...
describe ("Check if fields exist, and if they are named properly - ", function(){
    ...
    var EC = protractor.ExpectedConditions;
        ...
        it ("On event-add/menu2/ (clik on Swim), check if fields exist, and if they are named properly", function() {
            console.log("On event-add/menu2/ (clik on Swim), check if fields exist, and if they are named properly");
            //klik na Activity
            ...
            //Nacin da proveri text
            browser.wait(EC.presenceOf(eventAdd.categoris), 10000);
            expect(eventAdd.categoris.get(0).getText()).toBe('Recovery swim');
            expect(eventAdd.categoris.get(1).getText()).toBe('Strength swim');
            expect(eventAdd.categoris.get(2).getText()).toBe('Speed swim');
            expect(eventAdd.categoris.get(3).getText()).toBe('Technique swim');
            expect(eventAdd.categoris.get(4).getText()).toBe('Endurance swim');
            expect(eventAdd.categoris.get(5).getText()).toBe('Race pace swim');
            expect(eventAdd.categoris.get(6).getText()).toBe('Open water swim');
            expect(eventAdd.categoris.get(7).getText()).toBe('Other Swim');
            //klik na Recovery swim
            eventAdd.categoris.get(0).click();
